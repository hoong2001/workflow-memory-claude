...
- **Quiz the user before finalizing** — walk the proposed breakdown with them using the **question pattern** (always via `AskUserQuestion`: recommended answer + 3 alternatives + 1 custom) in `.claude/skills/_shared-conventions.md`. Confirm granularity, the blocking edges, and anything to merge or split. Don't publish the list until it's agreed.
